...
- Cloud, mídia pública para o Metricool: commit temporário do render na branch (repo público, raw.githubusercontent.com passa no proxy), agendar e remover o arquivo em seguida. Exige `git add -f` (o .gitignore barra mídia) com autorização do usuário. **Por isso este repo deve ser público** (confirmado público em 2026-08-18). Atenção: o proxy da sessão cloud bloqueia raw.githubusercontent.com de dentro do container; a rota continua válida porque quem busca a URL é o CDN do Metricool, mas para conferir a URL de dentro da sessão é preciso liberar raw.githubusercontent.com nos domínios do environment.
